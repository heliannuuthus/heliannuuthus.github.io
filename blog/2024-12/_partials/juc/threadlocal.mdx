...
- `ThreadLocal` 的内存泄漏问题
  ...
  - `ThreadLocalMap` 使用:term[**弱引用**]{./blog/terms/java#weak-reference}，如果触发 GC 会第一时间回收弱引用对象，此时的 `ThreadLocalMap` 中的 `Entry` 的 `Key` 会为 `null`，此时 `Entry` 的 `value` 会一直存在，直到线程结束。
