...
import { Center } from "@components/Image";
...
:::step{title="时间过 1s 后，增量请求为 10，请求总数为 10"}

1. 初始化桶，此时整个 HashMap 为空，可视为当前没有消耗令牌（即存在 100 个令牌）
2. 接收到请求，将当前时间戳对应的 key 设置为申请的令牌数
3. 当请求数大于等于限制时，拒绝请求
...
:::step{title="第 6 秒统计，过去 6 秒请求总数为 65, 拒绝 5 个请求"}
...
:text[第 2，4，6 桶位存在被拒绝请求，用其他颜色替代]{type="danger"}

:::

:::step{title="第 10 秒统计，过去的 10 秒请求总数为 109, 拒绝 9 个请求"}
...
:text[第 2，4，6，7，8 桶位存在被拒绝请求，用其他颜色替代]{type="danger"}

:::

:::step{title="第 12 秒统计，过去的 12 秒请求总数为 110, 拒绝 10 个请求"}
...
:text[第 1，2 桶位被覆盖，用其他的颜色表示]{type="danger"}
...
:::collapse{title="SlidingWindowRateLimiter.java"}
