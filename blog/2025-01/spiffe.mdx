...
**_:hint[spiffe://]{#协议名称，固定前缀}_:hint[trust-domain]{#信任域，通常是组织的或集群的域名}_:hint[/path/service-name]{title="工作负载的标识和路径"}_**
...
:::collapse{title="psat 的自证以及签发 SVID 的数据包"}
...
:::collapse{title="x509pop 的 Challenge 以及签发 SVID 的数据包"}
