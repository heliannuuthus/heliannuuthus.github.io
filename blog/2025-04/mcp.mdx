...
需要 MCP Server 自行实现 [:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc8414) 定义的 `/.well-known/oauth-authorization-server` 端点为 MCP Client 提供认证的*元数据信息*。
当 MCP Client 需要集成多个 MCP Server 时，并不能提前知晓 MCP Server 的认证方式以及*身份服务提供商*，故而 MCP Client 必须支持 [:hint[**rfc7591**]{id="OAuth 2.0 Dynamic Client Registration Protocol"}](https://datatracker.ietf.org/doc/html/rfc7591) 动态注入能力，这样即可无缝集成多个 MCP Server。

:::nerd

如果 Client 泄漏，那么如何保证 Client 不被其他人冒用呢？\
在 [:hint[**rfc9700**]{id="Best Current Practice for OAuth 2.0 Security"}](https://datatracker.ietf.org/doc/html/rfc9700#name-insufficient-redirection-ur) 中指出可通过**强校验 Redirect URI** 的手段来保证客户端不会被冒用。
...
- **应该**支持 [:hint[**rfc7591**]{id="OAuth 2.0 Dynamic Client Registration Protocol"}](https://datatracker.ietf.org/doc/html/rfc7591)

**MCP Client 和 MCP Server** 须满足：

- MCP Client **必须**实现 [:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc8414)
- MCP Server **应该**支持 [:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc7591)

## 授权流程

该节定义在传统 OAuth 流程中如何进行身份认证，以及在
[服务器元数据发现](#服务器元数据发现) ([:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc8414))
和[客户端动态注册](#客户端动态注册) ([:hint[**rfc7591**]{id="OAuth 2.0 Dynamic Client Registration Protocol"}](https://datatracker.ietf.org/doc/html/rfc7591))
...
- :hint[**authorization_code**]{#授权码模式}: 需要用户登录授权，用于客户端代表用户本身去完成一些操作。
- :hint[**client_credentials**]{#客户端凭据模式}: 不需要用户登录授权，用于客户端代表自己去做一些*自动化的操作*。
...
在 :hint[**rfc9700**]{id="Best Current Practice for OAuth 2.0 Security"} 中声明，公开的客户端（_通常没有应用密钥_）应使用 PKCE 模式发起认证，并且认证服务器应该进行**强制校验**。
...
> MCP Client **必须支持**获取和解析 [:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc8414) 返回的数据，\
...
1. 访问凭证**必须**按照 :hint[**rfc9700**]{id="Best Current Practice for OAuth 2.0 Security"} 的最佳实践进行存储保管
...
- 在 MCP 中的 OAuth 主要为了解决 MCP Client 和 MCP Server 在使用*远程调用*时，**身份认证**以及**权限控制**的问题
  ...
  1. MCP Client **必须**支持 [:hint[**rfc8414**]{id="OAuth 2.0 Authorization Server Metadata"}](https://datatracker.ietf.org/doc/html/rfc8414) 来发现 MCP Server 的认证方式，获取到*应用信息注册端点*以及*授权端点*
  2. MCP Client **应该**支持 [:hint[**rfc7591**]{id="OAuth 2.0 Dynamic Client Registration Protocol"}](https://datatracker.ietf.org/doc/html/rfc7591) 来向 OAuth Server 获取/动态注册自己作为应用的信息
