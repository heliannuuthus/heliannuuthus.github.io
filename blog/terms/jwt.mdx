...
- :ctip[**alg**]{title="algorithm"}：算法，在 :term[jwa]{./blog/terms/jwt#jwa} 中定义，在 :term[jws]{./blog/terms/jwt#jws} 和 :term[jwe]{./blog/terms/jwt#jwe} 中值不一致。
...
- JOSE Header：`alg`、`cty`、`typ` 参考 [JWT-JOSE Header](#jwt)
  - :ctip[**jku**]{title="JWK Set URL"}:ctip[(optional)]{title="可选的字段"}：是一个 :term[uri]{./blog/terms/web#uri}，指向一组 JSON 编码的公共密钥资源，其中一个与 jws 的签名密钥相关联。
  ...
  - :ctip[**x5c**]{title="X.509 Certificate Chain"}:ctip[(optional)]{title="可选的字段"}：X.509 证书/证书链，以 JSON 格式表示，单个证书以 Base64(NoUrl)-:term[der]{./blog/terms/crypto#der} 编码的形式存放。
