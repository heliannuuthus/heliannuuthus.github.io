...
- [ECDH-ESWithAesKeyWrap](https://www.rfc-editor.org/rfc/rfc8017)：区别于 ECDH-ES，使用 Diffie-Hellman 算法交换出来的密钥，
  ...
  - :ctip[**epk**]{title="Ephemeral Public Key"}:ctip[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：用于 Diffie-Hellman 算法交换[[jwk]](/blog/terms/jwt#jwk) 格式的临时公钥
...
- [RSA](https://www.rfc-editor.org/rfc/rfc8170)：RSA 密钥，e.g. `RSA`
- [EC](https://www.rfc-editor.org/rfc/rfc8422)：椭圆曲线密钥，是 SECP 系列的椭圆曲线，e.g. `EC`
- [OKP](https://www.rfc-editor.org/rfc/rfc8037)：椭圆曲线密钥，目前的实现是 Ed25519 密钥，e.g. `EdDSA`
- [oct](https://www.rfc-editor.org/rfc/rfc3826)：对称密钥，目前的特指 AES 密钥，e.g. `oct`
...
- JOSE Header：`alg`、`cty`、`typ` 参考 [JWT-JOSE Header](#jwt)
  - :ctip[`jku`]{title="JWK Set URL"}:ctip[(optional)]{title="可选的字段"}：是一个 [[URI]](/blog/terms/web#uri)，指向一组 JSON 编码的公共密钥资源，其中一个与 jws 的签名密钥相关联。
    如果使用 HTTP 获取密钥，则必须使用传输层安全协议保护（TLS）
  - :ctip[`jwk`]{title="JSON Web Key"}:ctip[(optional)]{title="可选的字段"}：用于数字签名的 JWK 的 JSON 对象
  - :ctip[`kid`]{title="Key ID"}:ctip[(optional)]{title="可选的字段"}：标识着当前 jws 被哪一个 jwk 保护（签名），通常在 **jwk set** 中使用 `kid` 找到对应的 jwk 进行验证。
  - :ctip[`x5u`]{title="X.509 URL"}:ctip[(optional)]{title="可选的字段"}：X.509 证书/证书链的 URL 地址，当前证书格式为 PEM，且证书的公钥必须与 jws 的签名私钥匹配，该证书可用于查找上级证书
    该证书的获取若使用 HTTP 协议，则必须使用传输层安全协议保护（TLS）
  - :ctip[`x5c`]{title="X.509 Certificate Chain"}:ctip[(optional)]{title="可选的字段"}：X.509 证书/证书链，以 JSON 格式表示，单个证书以 Base64(NoUrl)-[[der]](/blog/terms/crypto#der) 编码的形式存放。
  - :ctip[`x5t`]{title="X.509 Certificate SHA-1 Thumbprint"}:ctip[(optional)]{title="可选的字段"}：X.509 证书以 Base64Url 编码的 SHA-1 摘要
  - :ctip[`x5t#S256`]{title="X.509 Certificate SHA-256 Thumbprint"}:ctip[(optional)]{title="可选的字段"}：X.509 证书以 Base64Url 编码的 SHA-256 摘要
  - :ctip[`crit`]{title="Critical"}:ctip[(optional)]{title="可选的字段"}：声明一个字符串数组，用于表示该 jws 的 JOSE Header 内一些字段是验证方必须验证的，若验证方验证不了则应当拒绝当前 jws。
