...
description: 互联网内用于标识资源的标准字符串，通常包含 [[URL]](/blog/terms/web#url) 和 [[URN]](/blog/terms/web#urn) 两种形式
---

> URI（Uniform Resource Identifier，统一资源标识符）是一个用于标识互联网内资源的字符串。它是一个通用的资源标识符系统，包含了 [[URL]](/blog/terms/web#url) 和 [[URN]](/blog/terms/web#urn) 两种形式。
...
> URL（Uniform Resource Locator，统一资源定位符）是互联网内用于定位资源的标准地址格式。它是 [[URI]](/blog/terms/web#uri) 的一个子集，专门用于准确描述如何访问某个资源。

## URL 的格式和组成

_:ctip[https://]{title="协议"}:ctip[www.example.com]{title="主机名"}:ctip[:443]{title="端口号"}:ctip[/path/to/resource]{title="路径"}:ctip[?key=value]{title="查询参数"}:ctip[#section]{title="片段标识符"}_
...
slug: urlencode
title: URL 编码
description: 将非 ASCII 字符转换为 ASCII 字符集中的字符，通常用于 [[URL]](/blog/terms/web#url) 中。
