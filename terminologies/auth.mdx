...
- :hint[**typ**]{#type}:token 的类型，如果存在最好指定为 **JWT**，表示当前是 JWT token
- :hint[**cty**]{#content-type}:内容类型，当使用嵌套的 jws 或者 jwe 时，必须指定为 **JWT**（建议使用大写字母），反之常规情况**不推荐使用**
- :hint[**alg**]{#algorithm}:算法，在 :term[jwa]{./terms/auth#jwa} 中定义，在 :term[jws]{./terms/auth#jws} 和 :term[jwe]{./terms/auth#jwe} 中值不一致。
...
- Registered Claims：由 IANA 定义的声明，例如：
  - :hint[**iss**]{#issuer}：签发者, 签发 token 的实体
  - :hint[**sub**]{#subject}：主题, token 的主体信息，通常是用户标识
  - :hint[**aud**]{#audience}：受众, token 的接收者，验证 token 方
  - :hint[**exp**]{#expiration}：过期时间, token 的过期时间
  - :hint[**nbf**]{#not-before}：生效时间, token 的生效时间
  - :hint[**iat**]{#issued-at}：签发时间, token 的签发时间
  - :hint[**jti**]{#jwt-id}：token 的唯一标识
...
> jwe 中 alg 字段通常用于指定是否包装 :hint[`cek`]{title="Content Encryption Key"}，如果是 `dir` 则直接使用 `cek` 和 `enc` 字段指定的算法进行加密
...
- [AesGCMKeyWrap](https://www.rfc-editor.org/rfc/rfc8017)：区别于 AesKeyWrap，使用 GCM 模式进行加密，通常 Header 中伴随 `iv` 和 `tag` 使用。
  ...
  - :hint[**iv**]{title="Initialization Vector"}:hint[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：初始化向量，通常为 96 位，使用 Base64Url 编码
  - :hint[**tag**]{title="Authentication Tag"}:hint[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：认证标签，通常为 128 位，使用 Base64Url 编码
...
- [ECDH-ESWithAesKeyWrap](https://www.rfc-editor.org/rfc/rfc8017)：区别于 ECDH-ES，使用 Diffie-Hellman 算法交换出来的密钥，
  ...
  - :hint[**epk**]{title="Ephemeral Public Key"}:hint[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：用于 Diffie-Hellman 算法交换:term[jwk]{./terms/auth#jwk} 格式的临时公钥
  - :hint[**apu**]{title="Agreement Party UInfo"}:hint[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：发送方的附加信息，会在 ConcateKDF 中使用
  - :hint[**apv**]{title="Agreement Party VInfo"}:hint[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：接收方的附加信息，会在 ConcateKDF 中使用
- [PBES2](https://www.rfc-editor.org/rfc/rfc8018)：使用 PBKDF2 密码生成算法加上散列函数输出指定长度的密钥，对 cek 进行包装。
  ...
  - :hint[**p2c**]{title="PBES2 Count"}：PBKDF2 的迭代次数，目前推荐次数为 60,000 次，最小次数为 1,000 次
  - :hint[**p2s**]{title="PBES2 Salt"}：PBKDF2 的盐值，最小长度为 8 字节，每次加密必须使用不同的盐值，最后使用 Base64Url 编码。最后使用的盐值 (UTF-8(alg) || 0x00 || salt)
...
- JOSE Header：`alg`、`cty`、`typ` 参考 [JWT-JOSE Header](#jwt)
  - :hint[**jku**]{title="JWK Set URL"}:hint[(optional)]{#可选的字段}：是一个 :term[uri]{./terms/web#uri}，指向一组 JSON 编码的公共密钥资源，其中一个与 jws 的签名密钥相关联。
    如果使用 HTTP 获取密钥，则必须使用传输层安全协议保护（TLS）
  - :hint[**jwk**]{title="JSON Web Key"}:hint[(optional)]{#可选的字段}：用于数字签名的 JWK 的 JSON 对象
  - :hint[**kid**]{title="Key ID"}:hint[(optional)]{#可选的字段}：标识着当前 jws 被哪一个 jwk 保护（签名），通常在 **jwk set** 中使用 `kid` 找到对应的 jwk 进行验证。
  - :hint[**x5u**]{title="X.509 URL"}:hint[(optional)]{#可选的字段}：X.509 证书/证书链的 URL 地址，当前证书格式为 PEM，且证书的公钥必须与 jws 的签名私钥匹配，该证书可用于查找上级证书
    该证书的获取若使用 HTTP 协议，则必须使用传输层安全协议保护（TLS）
  - :hint[**x5c**]{title="X.509 Certificate Chain"}:hint[(optional)]{#可选的字段}：X.509 证书/证书链，以 JSON 格式表示，单个证书以 Base64(NoUrl)-:term[der]{./terms/crypto#der} 编码的形式存放。
  - :hint[**x5t**]{title="X.509 Certificate SHA-1 Thumbprint"}:hint[(optional)]{#可选的字段}：X.509 证书以 Base64Url 编码的 SHA-1 摘要
  - :hint[**x5t#S256**]{title="X.509 Certificate SHA-256 Thumbprint"}:hint[(optional)]{#可选的字段}：X.509 证书以 Base64Url 编码的 SHA-256 摘要
  - :hint[**crit**]{title="Critical"}:hint[(optional)]{#可选的字段}：声明一个字符串数组，用于表示该 jws 的 JOSE Header 内一些字段是验证方必须验证的，若验证方验证不了则应当拒绝当前 jws。
