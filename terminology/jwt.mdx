...
> 通常我们见到的都是用 `.` 分隔的三段(:term[jws]{./terms/jwt.mdx#jws})或五段(:term[jwe]{./terms/jwt.mdx#jwe}) Base64Url 编码的 JSON 对象。

## JWT 的格式和组成

> 用 `.` 分隔的三段(:term[jws]{./terms/jwt.mdx#jws})或五段(:term[jwe]{./terms/jwt.mdx#jwe}) Base64Url 编码的 JSON 对象
...
- :ctip[**alg**]{#algorithm}:算法，在 :term[jwa]{./terms/jwt#jwa} 中定义，在 :term[jws]{./terms/jwt#jws} 和 :term[jwe]{./terms/jwt#jwe} 中值不一致。
...
- [ECDH-ESWithAesKeyWrap](https://www.rfc-editor.org/rfc/rfc8017)：区别于 ECDH-ES，使用 Diffie-Hellman 算法交换出来的密钥，
  ...
  - :ctip[**epk**]{title="Ephemeral Public Key"}:ctip[(required)]{title="使用该算法，该字段必须存在于 Header 中"}：用于 Diffie-Hellman 算法交换:term[jwk]{./terms/jwt#jwk} 格式的临时公钥
